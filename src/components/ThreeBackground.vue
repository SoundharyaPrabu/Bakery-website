...
export default {
  mounted() {
    ...
    const sprinkleMaterial = new THREE.PointsMaterial({ color: 0xff69b4, size: 0.7, transparent: true, opacity: 0.8 });
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const mat = new THREE.MeshPhongMaterial({ color: sweetColors[i % sweetColors.length], shininess: 100 });
